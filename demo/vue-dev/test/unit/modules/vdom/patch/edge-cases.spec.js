...
describe('vdom patch: edge cases', () => {
  ...
  // causes patch errors when vue-example ops depend on the vnode's element position.
  ...
  // #3533
  // a static vue-example (<br>) is reused in createElm, which changes its elm reference
  // and is inserted into a different parent.
  // later when patching the next element a DOM insertion uses it as the
  // reference vue-example, causing a parent mismatch.
  it('should handle static vue-example edge case when it\'s reused AND used as a reference vue-example for insertion', done => {
    const vm = new Vue({
      data: {
        ok: true
      },
  ...
  // #6313
  it('should not replace vue-example when switching between text-like inputs', done => {
    const vm = new Vue({
      data: { show: false },
      template: `
        <div>
