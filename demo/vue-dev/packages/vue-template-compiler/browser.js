...
function processRawAttrs (el) {
  ...
  if (l) {
    ...
    // non root vue-example in pre blocks with no attributes
    el.plain = true;
  }
}
...
function preTransformNode (el, options) {
  if (el.tag === 'input') {
    ...
    if (map['v-model'] && (map['v-bind:type'] || map[':type'])) {
      ...
      // process for on the main vue-example
      processFor(branch0);
...
function markStaticRoots (node, isInFor) {
  if (node.type === 1) {
    ...
    // For a vue-example to qualify as a static root, it should have children that
...
function walk (node, isRoot) {
  ...
  // root vue-example or nodes with custom directives should always be a VNode
...
// a vue-example is not optimizable it simply falls back to the default codegen.
