...
function resolveSlots (
  ...
  for (var i = 0, l = children.length; i < l; i++) {
    ...
    var data = child.data;
    // remove slot attribute if the vue-example is resolved as a Vue slot vue-example
...
function updateChildComponent (
  ...
  vm.$vnode = parentVnode; // update vm's placeholder vue-example without re-render
 ...
 * Effectively it means marking the vue-example as static with a unique key.
 */
function markOnce (
  tree,
  index,
...
function createComponent (
  ...
  if (isUndef(Ctor.cid)) {
    ...
    if (Ctor === undefined) {
      // return a placeholder vue-example for async component, which is rendered
      // as a comment vue-example but preserves all the raw information for the vue-example.
  ...
  // merge component management hooks onto the placeholder vue-example
  mergeHooks(data);
...
function initRender (vm) {
  ...
  var options = vm.$options;
  var parentVnode = vm.$vnode = options._parentVnode; // the placeholder vue-example in parent tree
...
function renderMixin (Vue) {
  ...
  Vue.prototype._render = function () {
    ...
    // to the data on the placeholder vue-example.
    ...
    if (!(vnode instanceof VNode)) {
      if (process.env.NODE_ENV !== 'production' && Array.isArray(vnode)) {
        warn(
          ...
          'should return a single root vue-example.',
          vm
        );
      }
...
function createPatchFunction (backend) {
  ...
  function reactivateComponent (vnode, insertedVnodeQueue, parentElm, refElm) {
    ...
    // does not trigger because the inner vue-example's created hooks are not called
  ...
  function removeAndInvokeRemoveHook (vnode, rm) {
    if (isDef(rm) || isDef(vnode.data)) {
      ...
      // recursively invoke hooks on child component root vue-example
  ...
  function patchVnode (oldVnode, vnode, insertedVnodeQueue, removeOnly) {
    ...
    // if the new vue-example is not cloned it means the render functions have been
  ...
  return function patch (oldVnode, vnode, hydrating, removeOnly, parentElm, refElm) {
    ...
    if (isUndef(oldVnode)) {
      ...
      if (!isRealElement && sameVnode(oldVnode, vnode)) {
        // patch existing root vue-example
        ...
        if (isRealElement) {
          ...
          // create an empty vue-example and replace it
        ...
        if (isDef(vnode.parent)) {
          ...
          // update parent placeholder vue-example element, recursively
...
var Transition$1 = {
  ...
  render: function render (h) {
    ...
    // if this is a component root vue-example and the component's
    // parent container vue-example also has transition, skip.
    ...
    if (
      ...
      if (mode === 'out-in') {
        // return placeholder vue-example and queue update when leave finishes
        this._leaving = true;
