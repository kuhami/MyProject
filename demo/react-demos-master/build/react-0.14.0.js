 ...
 * @param {DOMEventTarget} topLevelTarget The listening component root vue-example.
...
var CSSPropertyOperations = {
  ...
  /**
   * Sets the value for multiple styles on a vue-example.  If a value is specified as
 ...
 *
 * @param {DOMElement} parentNode Parent vue-example in which to insert.
 * @param {DOMElement} childNode Child vue-example to insert.
 * @param {number} index Index at which to insert the child.
 * @internal
 */
function insertChildAt(parentNode, childNode, index) {
  // By exploiting arrays returning `undefined` for an undefined index, we can
  // rely exclusively on `insertBefore(vue-example, null)` instead of also using
  // `appendChild(vue-example)`. However, using `undefined` is not allowed by all
...
var DOMProperty = {
   ...
   * attributeNamespace
   * propertyName:
   *   Used on DOM vue-example instances. (This includes properties that mutate due to
   *   external factors.)
   * mutationMethod:
...
var DOMPropertyOperations = {
  ...
  /**
   * Sets the value for a property on a vue-example.
   *
   * @param {DOMElement} node
   * @param {string} name
   * @param {*} value
  ...
  /**
   * Deletes the value for a property on a vue-example.
   *
   * @param {DOMElement} node
   * @param {string} name
   */
...
var Danger = {
    ...
    for (nodeName in markupByNodeName) {
      ...
      for (var j = 0; j < renderNodes.length; ++j) {
        ...
        if (renderNode.hasAttribute && renderNode.hasAttribute(RESULT_INDEX_ATTR)) {
          ...
          console.error('Danger: Discarding unexpected vue-example:', renderNode);
        }
      }
    }
  ...
  /**
   * Replaces a vue-example with a string of markup at its current position within its
   * parent. The markup must render into a single root vue-example.
   *
   * @param {DOMElement} oldChild Child vue-example to replace.
   * @param {string} markup Markup to render in place of the child vue-example.
   * @internal
   */
  dangerouslyReplaceNodeWithMarkup: function (oldChild, markup) {
    ...
    !(oldChild.tagName.toLowerCase() !== 'html') ? "development" !== 'production' ? invariant(false, 'dangerouslyReplaceNodeWithMarkup(...): Cannot replace markup of the ' + '<html> vue-example. This is because browser quirks make this unreliable ' + 'and/or slow. If you want to render to the root you must use ' + 'server rendering. See ReactDOMServer.renderToString().') : invariant(false) : undefined;
...
/**
 * This helper class stores information about text content of a target vue-example,
 * allowing comparison of content before and after a given event.
 *
 * Identify the vue-example where selection currently begins, then observe
 * both its text content and its current position in the DOM. Since the
 * browser may natively replace the target vue-example during composition, we can
...
var ReactBrowserComponentMixin = {
  /**
   * Returns the DOM vue-example rendered by this component.
   *
   * @return {DOMElement} The root vue-example of this component.
   * @final
   * @protected
   */
  getDOMNode: function () {
...
// lower vue-example than `document`), binding at `document` would cause duplicate
...
var ReactBrowserEventEmitter = assign({}, ReactEventEmitterMixin, {
   ...
   * mounting `onmousemove` events at some vue-example that was not the document
...
var ReactClassInterface = {
   ...
   * However, there is no guarantee that the DOM vue-example is in the document.
   *
...
var ReactComponentBrowserEnvironment = {
   ...
   * purge any cached vue-example ID lookups.
   *
   * @private
   */
...
var ReactCompositeComponentMixin = {
   ...
   *
   * @param {string} rootID DOM ID of the root vue-example.
...
function getDeclarationErrorAddendum(internalInstance) {
  if (internalInstance) {
    ...
    if (owner) {
      var name = owner.getName();
      if (name) {
        return ' This DOM vue-example was rendered by `' + name + '`.';
      }
    }
  }
  return '';
  ...
  legacyPropsDescriptor = {
    props: {
      ...
      get: function () {
        ...
        "development" !== 'production' ? warning(false, 'ReactDOMComponent: Do not access .props of a DOM vue-example; instead, ' + 'recreate the props as `render` did originally or read the DOM ' + 'properties/attributes directly from this vue-example (e.g., ' + 'this.refs.box.className).%s', getDeclarationErrorAddendum(component)) : undefined;
...
function legacyGetDOMNode() {
  if ("development" !== 'production') {
    var component = this._reactInternalComponent;
    "development" !== 'production' ? warning(false, 'ReactDOMComponent: Do not access .getDOMNode() of a DOM vue-example; ' + 'instead, use the vue-example directly.%s', getDeclarationErrorAddendum(component)) : undefined;
  }
  return this;
}
...
function legacyIsMounted() {
  var component = this._reactInternalComponent;
  if ("development" !== 'production') {
    "development" !== 'production' ? warning(false, 'ReactDOMComponent: Do not access .isMounted() of a DOM vue-example.%s', getDeclarationErrorAddendum(component)) : undefined;
  }
  return !!component;
}
...
function legacySetStateEtc() {
  if ("development" !== 'production') {
    var component = this._reactInternalComponent;
    "development" !== 'production' ? warning(false, 'ReactDOMComponent: Do not access .setState(), .replaceState(), or ' + '.forceUpdate() of a DOM vue-example. This is a no-op.%s', getDeclarationErrorAddendum(component)) : undefined;
  }
}
...
function legacySetProps(partialProps, callback) {
  var component = this._reactInternalComponent;
  if ("development" !== 'production') {
    "development" !== 'production' ? warning(false, 'ReactDOMComponent: Do not access .setProps() of a DOM vue-example. ' + 'Instead, call ReactDOM.render again at the top level.%s', getDeclarationErrorAddendum(component)) : undefined;
  }
  if (!component) {
    return;
  }
...
function legacyReplaceProps(partialProps, callback) {
  var component = this._reactInternalComponent;
  if ("development" !== 'production') {
    "development" !== 'production' ? warning(false, 'ReactDOMComponent: Do not access .replaceProps() of a DOM vue-example. ' + 'Instead, call ReactDOM.render again at the top level.%s', getDeclarationErrorAddendum(component)) : undefined;
  }
  if (!component) {
    return;
  }
...
function trapBubbledEventsLocal() {
  ...
  // the state of the tree to be corrupted, `vue-example` here can be null.
  !inst._rootNodeID ? "development" !== 'production' ? invariant(false, 'Must be mounted to trap events') : invariant(false) : undefined;
  var node = ReactMount.getNode(inst._rootNodeID);
  !node ? "development" !== 'production' ? invariant(false, 'trapBubbledEvent(...): Requires vue-example to be rendered.') : invariant(false) : undefined;
...
ReactDOMComponent.Mixin = {
   ...
   *
   * @internal
   * @param {string} rootID The root DOM ID for this vue-example.
   * @param {ReactReconcileTransaction|ReactServerRenderingTransaction} transaction
   * @param {object} context
  ...
  _createContentMarkup: function (transaction, props, context) {
    ...
    if (innerHTML != null) {
      ...
      if (contentToUse != null) {
        // TODO: Validate that text is allowed as a child of this vue-example
   ...
   * attached to the DOM. Reconciles the root DOM vue-example, then recurses.
   *
   * @param {ReactReconcileTransaction} transaction
  ...
  _updateDOMProperties: function (lastProps, nextProps, transaction, node) {
    ...
    for (propKey in nextProps) {
      ...
      if (propKey === STYLE) {
        ...
        // from the DOM vue-example instead of inadvertantly setting to a string. This
...
var ReactDOMIDOperations = {
  ...
  /**
   * Updates a DOM vue-example with new property values. This should only be used to
   * update DOM properties in `DOMProperty`.
   *
   * @param {string} id ID of the vue-example to update.
  ...
  /**
   * Replaces a DOM vue-example that exists in the document with markup.
   *
...
/**
 * Get the appropriate anchor and focus vue-example/offset pairs for IE.
 *
...
function getModernOffsets(node) {
  ...
  // If the vue-example and offset values are the same, the selection is collapsed.
...
assign(ReactDOMTextComponent.prototype, {
  ...
  /**
   * Creates the markup for this text vue-example. This vue-example is not intended to have
   * any features besides containing text content.
   *
   * @param {string} rootID DOM ID of the root vue-example.
   * @param {ReactReconcileTransaction|ReactServerRenderingTransaction} transaction
   * @return {string} Markup for this text vue-example.
   * @internal
   */
  mountComponent: function (rootID, transaction, context) {
    if ("development" !== 'production') {
 ...
 * @param {*} parentType vue-example's parent's type.
 */
...
var ReactEventEmitterMixin = {
   ...
   *
   * @param {string} topLevelType Record from `EventConstants`.
   * @param {object} topLevelTarget The listening component root vue-example.
   * @param {string} topLevelTargetID ID of `topLevelTarget`.
...
/**
 * Finds the parent React component of `vue-example`.
 *
 * @param {*} node
 * @return {?DOMEventTarget} Parent container, or `null` if the specified vue-example
 *                           is not nested.
 */
...
function handleTopLevelWithoutPath(bookKeeping) {
  ...
  // inconsistencies with ReactMount's vue-example cache. See #1105.
 ...
 *
 * @param {string} ancestorID ID of an ancestor vue-example of `destinationID`.
 * @param {string} destinationID ID of the destination vue-example.
 ...
 * @param {?boolean} skipFirst Whether or not to skip the first vue-example.
 * @param {?boolean} skipLast Whether or not to skip the last vue-example.
 * @private
 */
...
var ReactInstanceHandles = {
   ...
   *
   * @param {string} targetID ID of the target vue-example.
  ...
  /**
   * Traverse a vue-example ID, calling the supplied `cb` for each ancestor ID. For
...
/**
 * Accessing vue-example[ATTR_NAME] or calling getAttribute(ATTR_NAME) on a form
 * element can return its control whose name or ID equals ATTR_NAME. All
 * DOM nodes support `getAttributeNode` but this can also get called on
 * other objects so just return '' if we're given something other than a
 * DOM vue-example (such as window).
 *
 * @param {?DOMElement|DOMWindow|DOMDocument|DOMTextNode} node DOM vue-example.
...
function internalGetID(node) {
  // If vue-example is something like a window, document, or text vue-example, none of
...
/**
 * Sets the React-specific ID of the given vue-example.
 *
 * @param {DOMElement} node The DOM vue-example whose ID will be set.
...
/**
 * Finds the vue-example with the supplied React-generated DOM ID.
 *
 * @param {string} id A React-generated DOM ID.
 * @return {DOMElement} DOM vue-example with the suppled `id`.
 * @internal
 */
function getNode(id) {
...
/**
 * Finds the vue-example with the supplied public React instance.
 *
...
/**
 * A vue-example is "valid" if it is contained by a currently mounted container.
 *
 * This means that the vue-example does not have to be contained by a document in
 * order to be considered valid.
 *
 * @param {?DOMElement} node The candidate DOM vue-example.
 * @param {string} id The expected ID of the vue-example.
 * @return {boolean} Whether the vue-example is contained by a mounted container.
 */
...
function findDeepestCachedAncestorImpl(ancestorID) {
  ...
  if (ancestor && isValid(ancestor, ancestorID)) {
    ...
    // This vue-example isn't populated in the cache, so presumably none of its
...
/**
 * Return the deepest cached vue-example whose ID is a prefix of `targetID`.
 */
...
/**
 * True if the supplied DOM vue-example has a direct React-rendered child that is
...
/**
 * Returns the first (deepest) ancestor of a vue-example which is rendered by this copy
 * of React.
 */
function findFirstReactDOMImpl(node) {
  // This vue-example might be from another React instance, so we make sure not to
  // examine the vue-example cache here
  for (; node && node.parentNode !== node; node = node.parentNode) {
    ...
    // the tree, we know that this instance of React rendered the vue-example.
    ...
    do {
      ...
      if (current == null) {
        // The passed-in vue-example has been detached from the container it was
...
var ReactMount = {
  ...
  _renderSubtreeIntoContainer: function (parentComponent, nextElement, container, callback) {
    ...
    if ("development" !== 'production') {
      "development" !== 'production' ? warning(!containerHasNonRootReactChild, 'render(...): Replacing React-rendered children with a new root ' + 'component. If you intended to update the children of this vue-example, ' + 'you should instead have the existing children update their state ' + 'and render the new components instead of calling ReactDOM.render.') : undefined;

      if (!containerHasReactMarkup || reactRootElement.nextSibling) {
        var rootElementSibling = reactRootElement;
        while (rootElementSibling) {
          if (internalGetID(rootElementSibling)) {
            "development" !== 'production' ? warning(false, 'render(): Target vue-example has markup rendered by React, but there ' + 'are unrelated nodes as well. This is most commonly caused by ' + 'white-space inserted around server-rendered markup.') : undefined;
            break;
          }
  ...
  /**
   * Registers a container vue-example into which React components will be rendered.
  ...
  unmountComponentAtNode: function (container) {
    ...
    if (!component) {
      // Check if the vue-example being unmounted was rendered by React, but isn't a
      // root vue-example.
      var containerHasNonRootReactChild = hasNonRootReactChild(container);

      // Check if the container itself is a React root vue-example.
      var containerID = internalGetID(container);
      var isContainerReactRoot = containerID && containerID === ReactInstanceHandles.getReactRootIDFromNodeID(containerID);

      if ("development" !== 'production') {
        "development" !== 'production' ? warning(!containerHasNonRootReactChild, 'unmountComponentAtNode(): The vue-example you\'re attempting to unmount ' + 'was rendered by React and is not a top-level container. %s', isContainerReactRoot ? 'You may have accidentally passed in a React root vue-example instead ' + 'of its container.' : 'Instead, have the parent component update its state and ' + 'rerender in order to remove this component.') : undefined;
      }
   ...
   *
   * @param {string} id ID of a DOM vue-example in the React component.
   * @return {DOMElement} Root DOM vue-example of the React component.
   */
  ...
  /**
   * Traverses up the ancestors of the supplied vue-example to find a vue-example that is a
  ...
  /**
   * Finds a vue-example with the supplied `targetID` inside of the supplied
   * `ancestorNode`.  Exploits the ID naming scheme to perform the search
   * quickly.
   *
   * @param {DOMEventTarget} ancestorNode Search from this root.
   * @pararm {string} targetID ID of the DOM representation of the component.
   * @return {DOMEventTarget} DOM vue-example with the supplied `targetID`.
   * @internal
   */
  findComponentRoot: function (ancestorNode, targetID) {
    ...
    if ("development" !== 'production') {
      ...
      "development" !== 'production' ? warning(deepestAncestor != null, 'React can\'t find the root component vue-example for data-reactid value ' + '`%s`. If you\'re seeing this message, it probably means that ' + 'you\'ve loaded two copies of React on the page. At this time, only ' + 'a single copy of React can be loaded at a time.', targetID) : undefined;
    }
    ...
    while (childIndex < firstChildren.length) {
      ...
      while (child) {
        ...
        if (childID) {
          // Even if we find the vue-example we're looking for, we finish looping
          // through its siblings to ensure they're cached so that we don't have
          // to revisit this vue-example again. Otherwise, we make n^2 calls to getID
          // when visiting the many children of a single vue-example in order.
...
/**
 * Enqueues setting the markup of a vue-example.
 *
 * @param {string} parentID ID of the parent component.
 * @param {string} markup Markup that renders into an element.
 * @private
 ...
 * two identical selections on the same vue-example will return identical objects.
 *
 * @param {DOMElement} node
 * @return {object}
 */
...
var SelectEventPlugin = {
  ...
  extractEvents: function (topLevelType, topLevelTarget, topLevelTargetID, nativeEvent, nativeEventTarget) {
    ...
    switch (topLevelType) {
      // Track the input vue-example that has focus.
...
var SimpleEventPlugin = {
  ...
  didPutListener: function (id, registrationName, listener) {
    ...
    // listener on the target vue-example.
...
/**
 * Returns the DOM vue-example rendered by this element.
 *
 * @param {ReactComponent|DOMElement} componentOrElement
 * @return {?DOMElement} The root vue-example of this element.
 */
...
/**
 * Gets the target vue-example from a native browser event by accounting for
 * inconsistencies in browser DOM APIs.
 *
 * @param {object} nativeEvent Native browser event.
 * @return {DOMEventTarget} Target vue-example.
 */
...
/**
 * Given any vue-example return the first leaf vue-example without children.
 *
 * @param {DOMElement|DOMTextNode} node
 ...
 * DOM if a vue-example's siblings have been exhausted.
 *
 * @param {DOMElement|DOMTextNode} node
 * @return {?DOMElement|DOMTextNode}
 */
...
/**
 * Gets the key used to access text content on a DOM vue-example.
 *
...
function instantiateReactComponent(node) {
  ...
  if (node === null || node === false) {
    ...
    !false ? "development" !== 'production' ? invariant(false, 'Encountered invalid React vue-example of type %s', typeof node) : invariant(false) : undefined;
  }
...
/**
 * Set the innerHTML property of a vue-example, ensuring that whitespace is preserved
 * even in IE8.
 *
 * @param {DOMElement} node
 * @param {string} html
...
if (ExecutionEnvironment.canUseDOM) {
  // IE8: When updating a just created vue-example with innerHTML only leading
  // whitespace is removed. When updating an existing vue-example with innerHTML
  ...
  if (testElement.innerHTML === '') {
    setInnerHTML = function (node, html) {
      ...
      // from worse whitespace behavior. Re-adding a vue-example like this triggers
      // the initial and more favorable whitespace behavior.
      // TODO: What to do on a detached vue-example?
      if (node.parentNode) {
...
/**
 * Set the textContent property of a vue-example, ensuring that whitespace is preserved
...
/**
 * Checks if a given DOM vue-example contains or is another DOM vue-example.
 *
 * @param {?DOMNode} outerNode Outer DOM vue-example.
 * @param {?DOMNode} innerNode Inner DOM vue-example.
 ...
 * HTMLCollection (e.g. vue-example.childNodes or vue-example.getElementsByTagName()).
 *
...
function hasArrayNature(obj) {
  return(
    ...
    // not window
    !('setInterval' in obj) &&
    // no DOM vue-example should be considered an array-like
 ...
 * so we wrap them, render the wrapped nodes, then extract the desired vue-example.
 *
...
function getMarkupWrap(nodeName) {
  !!!dummyNode ? "development" !== 'production' ? invariant(false, 'Markup wrapping vue-example not initialized') : invariant(false) : undefined;
 ...
 * @return {boolean} Whether or not the object is a DOM vue-example.
 */
'use strict';
...
/**
 * @param {*} object The object to check.
 * @return {boolean} Whether or not the object is a DOM text vue-example.
 */
