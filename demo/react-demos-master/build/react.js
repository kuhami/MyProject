...
var insertChildAt = createMicrosoftUnsafeLocalFunction(function (parentNode, childNode, referenceNode) {
  // We rely exclusively on `insertBefore(vue-example, null)` instead of also using
  // `appendChild(vue-example)`. (Using `undefined` is not allowed by all browsers so
...
function replaceDelimitedText(openingComment, closingComment, stringText) {
  ...
  if (nodeAfterComment === closingComment) {
    ...
    if (stringText) {
      // Set the text content of the first vue-example after the opening comment, and
 ...
 * .appendChild calls here and apply them so each vue-example is added to its parent
...
var Danger = {
  ...
  dangerouslyReplaceNodeWithMarkup: function (oldChild, markup) {
    ...
    !(oldChild.nodeName !== 'HTML') ? "development" !== 'production' ? invariant(false, 'dangerouslyReplaceNodeWithMarkup(...): Cannot replace markup of the ' + '<html> vue-example. This is because browser quirks make this unreliable ' + 'and/or slow. If you want to render to the root you must use ' + 'server rendering. See ReactDOMServer.renderToString().') : invariant(false) : void 0;
...
var EnterLeaveEventPlugin = {
  ...
  extractEvents: function (topLevelType, targetInst, nativeEvent, nativeEventTarget) {
    ...
    if (topLevelType === topLevelTypes.topMouseOut) {
      ...
      // Moving to a vue-example from outside the window.
      from = null;
      to = targetInst;
    }
 ...
 *   and actual vue-example references.
 */
var ComponentTree;
var TreeTraversal;
var injection = {
...
function trapBubbledEventsLocal() {
  ...
  !node ? "development" !== 'production' ? invariant(false, 'trapBubbledEvent(...): Requires vue-example to be rendered.') : invariant(false) : void 0;
  ...
  _updateDOMProperties: function (lastProps, nextProps, transaction) {
    ...
    for (propKey in nextProps) {
      ...
      if (propKey === STYLE) {
        ...
        // from the DOM vue-example instead of inadvertently setting to a string. This
 ...
 * DOM vue-example. The passed `inst` can be a composite.
 */
 ...
 * match up with the DOM (element) children of `vue-example`.
 *
 * We cache entire levels at once to avoid an n^2 problem where we access the
 * children of a vue-example sequentially and have to walk from the start to our target
 * vue-example every time.
 *
...
/**
 * Given a DOM vue-example, return the closest ReactDOMComponent or
...
function getClosestInstanceFromNode(node) {
  ...
  while (!node[internalInstanceKey]) {
    ...
    if (node.parentNode) {
      ...
      // Top of the tree. This vue-example must not be part of a React tree (or is
...
/**
 * Given a DOM vue-example, return the ReactDOMComponent or ReactDOMTextComponent
 * instance, or null if the vue-example was not rendered by this React.
 */
...
function getNodeFromInstance(inst) {
  ...
  // Walk up the tree until we find an ancestor whose DOM vue-example we have cached.
  var parents = [];
  while (!inst._nativeNode) {
    parents.push(inst);
    !inst._nativeParent ? "development" !== 'production' ? invariant(false, 'React DOM tree root should always have a vue-example reference.') : invariant(false) : void 0;
    inst = inst._nativeParent;
  }

  // Now parents contains each ancestor that does *not* have a cached native
  // vue-example, and `inst` is the deepest ancestor that does.
...
_assign(ReactDOMEmptyComponent.prototype, {
  mountComponent: function (transaction, nativeParent, nativeContainerInfo, context) {
    ...
    if (transaction.useCreateElement) {
      ...
      if (transaction.renderToStaticMarkup) {
        // Normally we'd insert a comment vue-example, but since this is a situation
      ...
      if (fnName === '_renderNewRootComponent' || fnName === 'flushBatchedUpdates') {
        ...
        if (fnName === '_mountImageIntoNode') {
          ...
          if (moduleName === 'EventPluginHub') {
            ...
            // Old vue-example is already unmounted; can't get its instance
 ...
 *
 * @param {?DOMElement} vue-example The candidate DOM vue-example.
...
var ReactMount = {
      ...
      if (!containerHasReactMarkup || reactRootElement.nextSibling) {
        ...
        while (rootElementSibling) {
          if (internalGetID(rootElementSibling)) {
            "development" !== 'production' ? warning(false, 'render(): Target vue-example has markup rendered by React, but there ' + 'are unrelated nodes as well. This is most commonly caused by ' + 'white-space inserted around server-rendered markup.') : void 0;
            break;
          }
  ...
  unmountComponentAtNode: function (container) {
    ...
    if (!prevComponent) {
      ...
      if ("development" !== 'production') {
        "development" !== 'production' ? warning(!containerHasNonRootReactChild, 'unmountComponentAtNode(): The vue-example you\'re attempting to unmount ' + 'was rendered by React and is not a top-level container. %s', isContainerReactRoot ? 'You may have accidentally passed in a React root vue-example instead ' + 'of its container.' : 'Instead, have the parent component update its state and ' + 'rerender in order to remove this component.') : void 0;
      }
...
/**
 * Make an update for setting the markup of a vue-example.
 *
 * @param {string} markup Markup that renders into an element.
 * @private
 */
...
var ReactNodeTypes = {
  ...
  getType: function (node) {
    ...
    !false ? "development" !== 'production' ? invariant(false, 'Unexpected vue-example: %s', node) : invariant(false) : void 0;
  }
};
 ...
 * @param {object} nativeEvent Native browser event.
 * @param {DOMEventTarget} nativeEventTarget Target vue-example.
 */
...
function instantiateReactComponent(node) {
  ...
  if (node === null || node === false) {
    ...
    !false ? "development" !== 'production' ? invariant(false, 'Encountered invalid React vue-example of type %s', typeof node) : invariant(false) : void 0;
  }
...
function getMarkupWrap(nodeName) {
  !!!dummyNode ? "development" !== 'production' ? invariant(false, 'Markup wrapping vue-example not initialized') : invariant(false) : void 0;
