...
function $_(cl,num) {
	return document.getElementsByClassName(cl)[""+num+""];
}
//正则表达式
var regs = {
...
window.onload = function() {
	...
	var resetPaw = $("resetPaw");
	var resetPaws = $("resetPaws");
	var pwd = $("pwd");
	var pwd2 = $("pwd2");
	var num =0,num1=1;
	var countDowns = $_("countDowns",num);
	var containerSign0 = $_("containerSign",num);
	var containerSign1 = $_("containerSign",num1);
	var successedReg = $_("successedReg",num);
	var navImg = $_("navImg",num);
	...
	//下一步，重置密码
	resetPaw.onclick = function() {
		if(checkMobile() && checkMessagecode()) {
			alert("下一步，重置密码成功！");
			
			containerSign0.style.display = "none";
			containerSign1.style.display = "block";
			navImg.src = 'img/2.jpg';
		} else {
			return false;
		}
	}
	//重置密码
	resetPaws.onclick = function() {
		if(checkPwd() && checkPwd2()) {
			alert("重置密码成功！");
			
			containerSign1.style.display = "none";
			successedReg.style.display = "block";
			navImg.src = 'img/3.jpg';
			var countDownTime = 3;
				var timer = setInterval(function(){
					if(countDownTime == 0){
						clearInterval(timer);
						...
						countDownTime--;
						countDowns.innerHTML = countDownTime;
					}
				},1000);
		} else {
			return false;
